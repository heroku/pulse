...
; statement4 (.createEPL admin "select count(*) from devent.win:time(30 sec) where ((cast(message?,string) regexp 'amqp_publish.*') and (cast(exchange?,string) regexp '(ps\\.run|service\\.needed).*')) output every 5 sec")
