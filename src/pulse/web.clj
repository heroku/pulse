...
(def graphs-index
  [[["nginx req/sec"       "nginx-requests-per-second"]
   ...
   [["api release/min"     "api-releases-per-minute"]
    ...
    ["scheduler run/min"   "scheduler-execs-per-minute"]]
...
(def graphs-routing
  [[["nginx req/sec"       "nginx-requests-per-second"]
   ...
   [["hermes req/sec"      "hermes-requests-per-second"]
    ...
    ["hermes econns"       "hermes-econns-per-minute"]]
   [["hermes req app/min"  "hermes-requests-apps-per-minute"]
    ["hermes h99 app/min"  "hermes-h99-apps-per-minute"]
    ["hermes econns app/min" "hermes-econns-apps-per-minute"]]
   [["hermes h10/min"      "hermes-h10-per-minute"]
    ...
    ["hermes h18/min"      "hermes-h18-per-minute"]]
   [["hermes h10 app/min"  "hermes-h10-apps-per-minute"]
    ["hermes h11 app/min"  "hermes-h11-apps-per-minute"]
    ["hermes h12 app/min"  "hermes-h12-apps-per-minute"]
    ["hermes h13 app/min"  "hermes-h13-apps-per-minute"]
    ["hermes h14 app/min"  "hermes-h14-apps-per-minute"]
    ["hermes h18 app/min"  "hermes-h18-apps-per-minute"]]
   [["services ls upd/min" "hermes-services-lockstep-updates-per-minute"]
    ["services ls conn/min" "hermes-services-lockstep-connections-per-minute"]
    ["services dconn/min"  "hermes-services-lockstep-disconnects-per-minute"]
    ["services ls ltcy"     "hermes-services-lockstep-mean-latency"]
    ["services ls staleness" "hermes-services-lockstep-mean-staleness"]]
   [["procs ls upd/min" "hermes-procs-lockstep-updates-per-minute"]
    ["procs ls conn/min" "hermes-procs-lockstep-connections-per-minute"]
    ["procs dconn/min"  "hermes-procs-lockstep-disconnects-per-minute"]
    ["procs ls ltcy"     "hermes-procs-lockstep-mean-latency"]
    ["procs ls staleness" "hermes-procs-lockstep-mean-staleness"]]
   [["domains ls upd/min" "hermes-domains-lockstep-updates-per-minute"]
    ["domains ls conn/min" "hermes-domains-lockstep-connections-per-minute"]
    ["domains dconn/min"  "hermes-domains-lockstep-disconnects-per-minute"]
    ["domains ls ltcy"     "hermes-domains-lockstep-mean-latency"]
    ["domains ls staleness" "hermes-domains-lockstep-mean-staleness"]]
   [["domain-groups ls upd/min" "hermes-domain-groups-lockstep-updates-per-minute"]
    ["domain-groups ls conn/min" "hermes-domain-groups-lockstep-connections-per-minute"]
    ["domain-groups dconn/min"  "hermes-domain-groups-lockstep-disconnects-per-minute"]
    ["domain-groups ls ltcy"     "hermes-domain-groups-lockstep-mean-latency"]
    ["domain-groups ls staleness" "hermes-domain-groups-lockstep-mean-staleness"]]
...
(def graphs-railgun
  [[["railgun running"     "railgun-running-count"]
   ...
   [["railgun s3 req/min"  "railgun-s3-requests-per-minute"]
    ...
    ["slug dwld fails/min" "railgun-slug-download-fails-per-minute"]]
   [["railgun excp/min"    "railgun-unhandled-exceptions-per-minute"]
...
(def graphs-api
  [[["api jobs/min"        "api-worker-jobs-per-minute"]
   ...
   [["api 500 resp/min"    "api-response-500-errors"]
    ["api 502 resp/min"    "api-response-502-errors"]
    ["api 503 resp/min"    "api-response-503-errors"]
    ["api 504 resp/min"    "api-response-504-errors"]]
